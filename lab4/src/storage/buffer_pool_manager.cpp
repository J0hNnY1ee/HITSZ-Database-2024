...
void BufferPoolManager::update_page(Page* page, PageId new_page_id, frame_id_t new_frame_id) {
    ...
    if (page->is_dirty_) {
        disk_manager_->write_page(page->get_page_id().fd,       // 文件描述符
                                  page->get_page_id().page_no,  // 页面编号
                                  page->get_data(),             // 页面数据
                                  PAGE_SIZE);                   // 页面大小
    ...
    std::memset(page->data_, 0, PAGE_SIZE);  // 清空页面数据
    page->id_ = new_page_id;                 // 更新页面 ID
    page->pin_count_ = 0;                    // 重置页面的 pin 计数
}
    ...
    if (it != page_table_.end()) {  // Page found in the page table
        ...
        if (find_victim_page(&frame_id)) {  // Found a victim page to replace
            ...
            if (victim_page.is_dirty()) {
                disk_manager_->write_page(victim_page.get_page_id().fd, victim_page.get_page_id().page_no,
...
void BufferPoolManager::flush_all_pages(int fd) {
    ...
    // 遍历 page_table_，找到与文件句柄 fd 匹配的页面
    for (const auto& [page_id, frame_id] : page_table_) {
        Page* page = &pages_[frame_id];
        if (page_id.fd != fd || page->get_page_id().page_no != INVALID_PAGE_ID) {
            continue;  // 如果文件句柄不匹配，跳过该页面
        }
        ...
        // 使用 const_cast 将 page_id 转换为非常量类型以调用 toString()
        std::cout << "Flushed page " << const_cast<PageId&>(page_id).toString() << " to disk." << std::endl;
    }
}
