...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    if (lseek(fd, offset_in_file, SEEK_SET) == -1) {
        std::cerr << "Error: Failed to seek to position for writing (page_no: " << page_no << ")" << std::endl;
        throw UnixError();  // lseek 出错时抛出异常
    }

    ssize_t bytes_written = write(fd, offset, num_bytes);
    if (bytes_written != num_bytes) {
        std::cerr << "Error: Incomplete write (expected: " << num_bytes << ", written: " << bytes_written << ")"
                  << std::endl;
        throw InternalError("DiskManager::write_page Error");
    }

    std::cout << "Page " << page_no << " written successfully." << std::endl;
}
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    off_t offset_in_file = page_no * PAGE_SIZE;
    if (lseek(fd, offset_in_file, SEEK_SET) == -1) {
        std::cerr << "Error: Failed to seek to position for reading (page_no: " << page_no << ")" << std::endl;
        throw UnixError();  // lseek 出错时抛出异常
    }
    ssize_t bytes_read = read(fd, offset, num_bytes);
    if (bytes_read != num_bytes) {
        std::cerr << "Error: Incomplete read (expected: " << num_bytes << ", read: " << bytes_read << ")" << std::endl;
        throw InternalError("DiskManager::read_page Error");
    }
    std::cout << "Page " << page_no << " read successfully." << std::endl;
}
...
void DiskManager::destroy_file(const std::string &path) {
    ...
    // 注意不能删除未关闭的文件
    if (!is_file(path)) {
        std::cerr << "Error: Cannot delete non-existent file: " << path << std::endl;
        throw FileNotFoundError(path);  
    }
    if (path2fd_.find(path) != path2fd_.end()) {
        std::cerr << "Error: Cannot delete file. The file is still open: " << path << std::endl;
        return;
    }
...
int DiskManager::open_file(const std::string &path) {
    ...
    if (path2fd_.find(path) != path2fd_.end()) {
        std::cout << "Warning: File already opened: " << path << std::endl;
        return path2fd_[path];  // 返回已打开文件的描述符
    }

    int fd = open(path.c_str(), O_RDWR | O_CREAT, 0644);
    if (fd == -1) {
        std::cerr << "Error: Failed to open file: " << path << std::endl;
        throw UnixError();  // 打开失败时抛出异常
    }
    path2fd_[path] = fd;
    fd2path_[fd] = path;

    std::cout << "File opened successfully: " << path << " (fd: " << fd << ")" << std::endl;
    return fd;  // 返回文件描述符
}
...
void DiskManager::close_file(int fd) {
    ...
    if (fd2path_.find(fd) == fd2path_.end()) {
        std::cerr << "Error: Attempt to close an unopened file (fd: " << fd << ")" << std::endl;
        return;
    }

    if (close(fd) == -1) {
        std::cerr << "Error: Failed to close file (fd: " << fd << ")" << std::endl;
        return;
    }
